...
import time
time.sleep(2)
print("Decryption is about to start:")
time.sleep(5) 
...
def main():
    ...
    decipher_as_receiver(ast.literal_eval(read_in_file("result/output_msg_crypt.txt")), 
...
#open and read the file after the appending:
 ...
 # the decryption function which takes the encrypted text as a parameter, and the public key (modulo, multiplicative) and the private key 
def decipher_as_receiver(ciphered_vector, modulo, multiplicative_to_mask, private_key_vector):
    ...
    print("Finished to decipher the text in " + str(time.process_time()-t) + " ms as a receiver.\n\n" +
          "Original text: " +
          str(deciphered_text))
    print("encrypted cipher text : " + str(ciphered_vector) )
    write_in_file("result/input_msg_decrypt.txt",deciphered_text)
...
if __name__ == "__main__":
        ...
        main()
        print("End of Decryption Program\n")
